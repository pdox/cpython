...
/* "RunFrames" are frames that exist while a function is running.
   ...
   moment, the Python call stack will contain 1 or more
...
/* Push a PyRunFrame corresponding to a frame */
...
/* Pop a PyRunFrame corresponding to a frame */
...
#define PyRunFrame_SetFrameRef(rf, frameobj) do { \
    (rf)->ref = (uintptr_t)(frameobj); \
    (rf)->f_locals = NULL; \
    (rf)->f_lasti = -1; \
} while (0)
