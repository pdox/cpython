...
stackdepth_push(basicblock ***sp, basicblock *b, int depth)
{
    ...
    if (!(b->b_startdepth < 0 || b->b_startdepth == depth)) {
        ...
        basicblock* todump[1024];
        int dc = 0;
        basicblock *entry = b;
        while (entry->b_list != NULL) entry = entry->b_list;
        todump[dc++] = entry;
        while (dc > 0) {
            b = todump[--dc];
            if (b->b_startdepth == -2) continue;
            fprintf(stderr, "%p: (start_depth = %d)\n", b, b->b_startdepth);
            b->b_startdepth = -2;
            for (int i = 0 ; i < b->b_iused; i++) {
                const char *opname = opcode_names[b->b_instr[i].i_opcode];
                if (b->b_instr[i].i_target != NULL) {
                    fprintf(stderr, "  %s %p\n", opname, b->b_instr[i].i_target);
                    if (b->b_instr[i].i_target->b_startdepth != -2) {
                        todump[dc++] = b->b_instr[i].i_target;
                    }
                } else {
                    fprintf(stderr, "  %s\n", opname);
                }
