...
void _Py_InitializeCore(const _PyCoreConfig *config)
{
    ...
    if ((p = Py_GETENV("PYJITDEBUGFILE")) && *p != '\0')
        Py_JITDebugFile = strdup(p);
    if ((p = Py_GETENV("PYJITSUPER")) && *p != '\0')
        Py_JITSuper = atoi(p);
    if ((p = Py_GETENV("PYJITEVALBREAKS")) && *p != '\0')
        Py_JITEvalBreaks = atoi(p);
    if ((p = Py_GETENV("PYJITPATCHPOINT")) && *p != '\0')
